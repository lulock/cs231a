...
def estimate_initial_RT(E):
    ...
    # translation is simply either u3 or -u3 where u3 is the third column vector of U
    T1 = np.array([U[:, 2]])
    T1 = T1.T
    T2 = -1*T1

    return np.append(R1, T1, axis=1), np.append(R2, T2, axis=1), np.append(R1, T2, axis=1), np.append(R2, T1, axis=1)
...
def linear_estimate_3d_point(image_points, camera_matrices):
    ...
    image_points  # Mx2 matrix
    camera_matrices  # Mx3x4
    A_sets = []
    for i, image in enumerate(image_points):
        pi_x = image[0]
        pi_y = image[1]
        mi = camera_matrices[i]
        row1 = pi_x*mi[2] - mi[0]
        row2 = pi_y*mi[2] - mi[1]
        A_sets = np.append(A_sets, row1)
        A_sets = np.append(A_sets, row2)

    # print(M)
    A = A_sets.reshape((2*M, 4))

    # obtain 3D points via svd
    U, s, Vt = np.linalg.svd(A)

    # H&Z 4.1 The Direct Linear Transformation (DLT) algorithm
    # h is the last column of V (which is the last row of V transpose)
    h = Vt[-1]
...
def reprojection_error(point_3d, image_points, camera_matrices):
    ...
    for i in range(len(image_points)):
        y = camera_matrices[i]@point_3d
        p_i = (1/y[2])*y[:2]
        e_i = p_i - image_points[i]
        error = np.append(error, e_i)

    # error = error.reshape((len(image_points), 2))
    # print(error)
    return error
    # raise Exception('Not Implemented Error')
...
def jacobian(point_3d, camera_matrices):
    ...
    p = camera_matrices@np.append(point_3d, 1.0)
    print(p)
